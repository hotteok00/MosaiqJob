...
def search_repos(q: str) -> str:
    data = server.api_get(f"{API_BASE}/search/repositories", params={"q": q}).json()
...
def search_code(q: str) -> str:
    data = server.api_get(f"{API_BASE}/search/code", params={"q": q}).json()
    total = data.get("total_count", 0)
    items = data.get("items", [])[:10]
...
def get_file(owner: str, repo: str, path: str) -> str:
    data = server.api_get(f"{API_BASE}/repos/{owner}/{repo}/contents/{path}").json()
...
def list_commits(owner: str, repo: str) -> str:
    ...
    items = data[:10]
    lines = [f"{owner}/{repo} 최근 커밋 ({len(items)}개)\n"]
    for i, commit_data in enumerate(items, 1):
